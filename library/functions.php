...
if ( ! function_exists( 'library_setup' ) ) :
/**
...
function library_setup() {
	...
	add_image_size( 'library-category-image', 450, 338, true );

	// This theme uses wp_nav_menu() in one location.
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'library' ),
		'login'   => esc_html__( 'Login', 'library' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'library_custom_background_args', array(
	 ...
	 */
	add_editor_style( array( 'assets/css/editor-style.css', library_fonts_url() ) );
...
if ( ! function_exists( 'library_fonts_url' ) ) :
/**
 * Register Google fonts
 *
 * @return string Google fonts URL for the theme.
 */
function library_fonts_url() {
	$fonts_url = '';
	$fonts     = array();
	$subsets   = 'latin,latin-ext';

	/* translators: If there are characters in your language that are not supported by Open Sans, translate this to 'off'. Do not translate into your own language. */
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'library' ) ) {
	...
	if ( 'off' !== _x( 'on', 'Montserrat font: on or off', 'library' ) ) {
	...
	if ( 'off' !== _x( 'on', 'Courgette font: on or off', 'library' ) ) {
		$fonts[] = 'Courgette:400';
	}

	/* translators: If there are characters in your language that are not supported by Roboto, translate this to 'off'. Do not translate into your own language. */
	if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'library' ) ) {
	...
	if ( 'off' !== _x( 'on', 'Raleway font: on or off', 'library' ) ) {
		$fonts[] = 'Raleway:400,100,300,500,600,700';
	}

	/* translators: If there are characters in your language that are not supported by Poppins, translate this to 'off'. Do not translate into your own language. */
	if ( 'off' !== _x( 'on', 'Poppins font: on or off', 'library' ) ) {
...
function library_scripts() {
	...
	wp_enqueue_style( 'library-fonts', library_fonts_url(), array(), null );
	...
	// Load custom js
	wp_enqueue_script( 'library-custom', get_template_directory_uri() . '/assets/js/custom' . $min . '.js', array(), '',true );

	// Load animation js
	wp_enqueue_script( 'library-animation', get_template_directory_uri() . '/assets/js/animation' . $min . '.js', array(), '',true );

	wp_enqueue_script( 'library-navigation', get_template_directory_uri() . '/assets/js/navigation' . $min . '.js', array(), '20151215', true );

	// Load the html5 shiv.
	wp_enqueue_script( 'library-html5', get_template_directory_uri() . '/assets/js/html5' . $min . '.js', array(), '3.7.3' );
	wp_script_add_data( 'library-html5', 'conditional', 'lt IE 9' );

	wp_enqueue_script( 'library-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix' . $min . '.js', array(), '20151215', true );
...
*/
function library_custom_sizes( $sizes ) {
    return array_merge( $sizes, array(
        'library-category-image' 		=> esc_html__( 'Gallery Thumbnail', 'library' ),
    ) );
}
add_filter( 'image_size_names_choose', 'library_custom_sizes' );
