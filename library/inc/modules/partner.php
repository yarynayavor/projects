...
if ( ! function_exists( 'library_add_partner' ) ) :
  /**
   * Add Partners
   *
   *@since library 0.3
   */
  function library_add_partner() {

    // Check if partner is enabled on frontpage
    $partner_enable = apply_filters( 'library_section_status', true, 'partner_enable' );
    if ( true !== $partner_enable ) {
      return false;
    }
    // Get Partners details
    $section_details = array();
    $section_details = apply_filters( 'library_filter_partner_details', $section_details );
    ...
    // Render Partners now.
    library_render_partner( $section_details );
  }
endif;
add_action( 'library_footer_content', 'library_add_partner', 10 );


if ( ! function_exists( 'library_get_partner_details' ) ) :
  /**
   * Partners details.
   *
   * @since library 0.3
   ...
   */
  function library_get_partner_details( $input ) {
...
add_filter( 'library_filter_partner_details', 'library_get_partner_details' );


if ( ! function_exists( 'library_render_partner' ) ) :
  /**
   ...
   *
   */
   function library_render_partner( $content_details = array() ) {
        $options = library_get_theme_options();  
